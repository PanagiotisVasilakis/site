...
const { vi } = Vitest as unknown as { vi: { mock: typeof Vitest.describe extends never ? any : (path: string, factory: () => any) => void } };
import { render, screen, waitFor, cleanup } from '@testing-library/react';
import userEvent from '@testing-library/user-event';
import BookingPage from '@/app/[locale]/book/page';
import BookingForm from '@/components/BookingForm';

// Use optional chaining to avoid runtime error if vi not injected (type fallback)
vi.mock('@/components/VillaLocationMap', () => ({
  default: (props: { locale?: string }) => <div data-testid="mock-villa-map">Map mock locale={props.locale}</div>
}));
vi.mock('next/image', () => ({
  __esModule: true,
  default: (props: { alt?: string; [k: string]: any }) => <img alt={props.alt || ''} {...props} />
}));
...
// Attempt manual cleanup using global if provided; otherwise rely on library auto cleanup
(globalThis as any).afterEach?.(() => cleanup());
...
describe('BookingPage (server component harness)', () => {
  ...
  it('shows initial validation errors and enables submit once fields valid', async () => {
    const ui = await BookingPage({ params: makeParams('en'), searchParams: makeSearchParams({ guests: '2', checkin: '2025-09-05', checkout: '2025-09-07' }) });
    render(ui as any);
  ...
  cleanup();
    const dateRange = { from: new Date('2025-09-10'), to: new Date('2025-09-12') };
    render(
      <BookingForm
