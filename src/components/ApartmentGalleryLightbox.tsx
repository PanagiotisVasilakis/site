...
export default function ApartmentGalleryLightbox({ photos, alts, springPreset='medium', enableHaptics=true, locale='en', labels }: Props) {
  ...
  <div className="flex items-center justify-between p-3 text-white text-sm">
    ...
    <div className="flex gap-2 pointer-events-auto">
      <button data-gallery-control className="bg-white/20 hover:bg-white/30 text-white border border-white/30 btn-sm pointer-events-auto" onClick={()=>{ prevRef.current?.(); triggerHaptic(6); }} aria-label={resolvedLabels.prev} title={resolvedLabels.prev}><span aria-hidden="true">◀</span><span className="sr-only">{resolvedLabels.prev}</span></button>
      <button data-gallery-control className="bg-white/20 hover:bg-white/30 text-white border border-white/30 btn-sm pointer-events-auto" onClick={()=>{ next(); triggerHaptic(6); }} aria-label={resolvedLabels.next} title={resolvedLabels.next}><span aria-hidden="true">▶</span><span className="sr-only">{resolvedLabels.next}</span></button>
      <button data-gallery-control className="bg-white/10 hover:bg-white/20 text-white border border-white/20 btn-sm pointer-events-auto" onClick={()=>{ hide(); triggerHaptic(4); }} aria-label={resolvedLabels.close} title={resolvedLabels.close}><span aria-hidden="true">✕</span><span className="sr-only">{resolvedLabels.close}</span></button>
    </div>
  </div>
  <div ref={viewerRef} className="relative flex-1 flex items-start justify-start overflow-hidden select-none" style={{ '--edge-left':'0','--edge-right':'0','--edge-top':'0','--edge-bottom':'0' } as CSSProperties}>
      <div ref={sliderRef} className="flex h-full w-full will-change-transform" style={{transition:'none'}}>
        {activePhotos.map((p,i)=>{ const alt=(alts && alts[p.altKey])||p.altKey; const active=i===index; const fadingOut=fadeFrom===i && fadeFrom!==index; return (
          <div key={`${p.src}-${i}`} className="w-full h-full flex-shrink-0 flex justify-center items-start transition-transform duration-150" ref={el=>{ if(active) activeWrapperRef.current=el; }}>
  ...
  <div className="p-3 flex gap-1 overflow-x-auto" aria-label={thumbnailsLabel}>{activePhotos.map((p,i)=>(<button data-gallery-control key={`${p.src}-thumb-${i}`} onClick={()=>show(i)} className={`relative w-16 h-12 rounded overflow-hidden ring-2 ${i===index?'ring-white':'ring-transparent'} focus:outline-none focus:ring-white`} aria-label={showPhotoLabel(i+1)}><span className="absolute inset-0"><Image src={p.src} alt="" fill sizes="64px" placeholder="blur" blurDataURL={GENERIC_BLUR} className="object-cover" /></span></button>))}</div>
  </div>)}</>);
}
