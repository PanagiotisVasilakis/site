...
export function withApiValidation<T>(
  ...
  return async (request: NextRequest, context: Record<string, unknown> = {}): Promise<NextResponse> => {
    try {
      // Validate request
  const validationResult = apiValidator.validateRequest(request, schema || z.any(), options);
      
      if (!validationResult.success && validationResult.errors) {
        return apiValidator.createValidationErrorResponse(validationResult.errors);
      }
      ...
      // This would require parsing the response body and validating against expected schema
...
export const ValidationHelpers = {
  validateAnalyticsEvents: (data: unknown) => {
    const EventsSchema = z.union([
      ApiSchemas.AnalyticsEvent,
      z.array(ApiSchemas.AnalyticsEvent).max(50),
    ]);
    
    return EventsSchema.safeParse(data);
  },

  validatePathParams: (category: string, slug?: string) => {
    const errors: string[] = [];
    
    if (!category || !/^[a-z0-9\-_]+$/.test(category) || category.length > 50) {
      errors.push('Invalid category parameter');
    }
    
    if (slug && (!/^[a-z0-9\-_]+$/.test(slug) || slug.length > 100)) {
      errors.push('Invalid slug parameter');
    }
    
    return { valid: errors.length === 0, errors };
  },

  sanitizeInput: (input: string, maxLength: number = 200): string => {
    return input
