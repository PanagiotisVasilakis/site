...
export class DevStoreBookingLookup implements BookingLookupProvider {
  async lookupByReference({ bookingRef, lastName }: LookupByRefParams): Promise<Booking | null> {
    const rec = guestStore.findBookingByReferenceAndLastName(bookingRef.trim(), lastName);
    if (!rec) return null;
    const user = rec.user_id ? guestStore.findUserById(rec.user_id) : undefined;
  ...
  async lookupByPhone({ phone, windowFromISO }: LookupByPhoneParams): Promise<Booking | null> {
    ...
    if (!n) return null;
    const user = guestStore.findUserByPhone(n.e164);
    if (!user) return null;
    const from = windowFromISO || new Date().toISOString().slice(0, 10);
    const booking = guestStore.findEligibleBookingForUser(user.id, from);
